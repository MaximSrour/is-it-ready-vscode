...
is-it-ready-vscode is a VS Code extension that runs formatting, linting, tests,
inventory, and security checks in one dashboard. It's written in TypeScript and
...
3. Run `npm run check` to ensure the project is clean
...
- `src/` - Extension source
  ...
  - `helpers.ts` - Extension helpers
  - `*.test.ts` - Unit tests
- Build output:
  - `dist/` - Compiled extension output
- Root config:
  - `package.json`, `tsconfig.json`, `eslint.config.mjs`,
    `prettier.config.cjs`, `knip.config.js`
